...
"""
    two_step_prais_winsten(X, y; intercept=true) -> PraisWinstenResult
...
"""
function two_step_prais_winsten(X, y; intercept::Bool = true,
...
function two_step_prais_winsten(formula::FormulaTerm, data; kwargs...)
    ...
    return two_step_prais_winsten(X, y; intercept=false, coefnames=cn, mf=mf, kwargs...)
end

"""
    iterated_prais_winsten(X, y; intercept=true, tol=1e-8, maxiter=100) -> PraisWinstenResult
...
"""
function iterated_prais_winsten(X, y; intercept::Bool = true, tol=1e-8, maxiter=100,
                               coefnames::Vector{String} = intercept ? ["(Intercept)"; ["x$i" for i in 1:size(X,2)]] : ["x$i" for i in 1:size(X,2)],
                               mf=nothing)
    X     = intercept ? hcat(ones(eltype(X), size(X,1)), X) : X
    rho   = estimate_rho(residuals(lm(X, y)))
...
function iterated_prais_winsten(formula::FormulaTerm, data; kwargs...)
    X, y, cn, mf = _extract_Xy(formula, data)
    return iterated_prais_winsten(X, y; intercept=false, coefnames=cn, mf=mf, kwargs...)
end
