...
"""
    hildreth_lu(X, y; intercept=true, nrho=200) -> HildrethLuResult
...
- `intercept`: if `true` (default), a constant column is prepended to `X`
               automatically.
- `nrho`     : number of grid points for rho in (-0.99, 0.99) (default `200`).
...
"""
function hildreth_lu(X, y; intercept::Bool = true, nrho=200,
...
function hildreth_lu(formula::FormulaTerm, data; kwargs...)
    ...
    return hildreth_lu(X, y; intercept=false, coefnames=cn, mf=mf, kwargs...)
end
