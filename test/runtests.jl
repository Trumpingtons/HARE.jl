...
σ_lin   = 0.5 .+ 0.3 .* abs.(x1)          # Glejser DGP: σ linear in |x1|
y_glej  = X * β_true .+ σ_lin .* randn(n)
data_glej = (y = y_glej, x1 = x1, x2 = x2)

ϵ    = randn(n)
u_ar = zeros(n)
for t in 2:n; u_ar[t] = 0.7 * u_ar[t-1] + ϵ[t]; end
y_ar = X * β_true .+ u_ar
data_ar = (y = y_ar, x1 = x1, x2 = x2)

u_hae = zeros(n)
for t in 2:n; u_hae[t] = 0.7 * u_hae[t-1] + σ[t] * ϵ[t]; end
...
@testset "two_step_glejser" begin
    ...
    # separate auxiliary regressors Z
...
# ── Consistency: iterated ≈ two-step (large n) ───────────────────────────────
...
# ── Formula metadata & newdata predict ───────────────────────────────────────
@testset "formula metadata and predict(newdata)" begin
    ...
    for (name, fit) in estimators
        @testset "$name" begin
            ...
            # matrix fit: generic coefnames
            @test coefnames(mm) == ["x1", "x2", "x3"]
            ...
            # predict with newdata DataFrame
            ŷ_formula = predict(mf, newdata)
            ŷ_matrix  = predict(mf, Xnew)
            @test length(ŷ_formula) == 5
            @test ŷ_formula ≈ ŷ_matrix
...
@testset "wald_test" begin
    ...
    # single restriction matches tstat²/1 ~ F(1, n-k)
    R1 = reshape([0.0, 1.0, 0.0], 1, k)
    w1 = wald_test(m, R1)
    @test w1.df == 1
    @test w1.stat ≈ tstat(m)[2]^2 atol=1e-10

    # dimension mismatch errors
    @test_throws DimensionMismatch wald_test(m, ones(2, k+1))
...
# ── Likelihood ratio test (Beach–MacKinnon only) ──────────────────────────────
...
# ── tstat / pvalues / confint: dist keyword ───────────────────────────────────
